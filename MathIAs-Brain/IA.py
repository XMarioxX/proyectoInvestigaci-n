    ...
    if métodoIn == "exit" :
        ...
        try:
            ...
            if easy_int > 0:
                ...
                derivada = "y = arcocoseno(sin x ^ 2 - cos x ^ 2)"
                resultado = "(2x(cos x ^ 2 + sin x ^ 2))/(sqrt(sin(2x ^ 2)))"
                print(derivada)

                completion = client.chat.completions.create(
                    model="gpt-4",
                    messages=[
                        {"role": "user", "content": "Dame la resolución paso a paso obligatoriamente de la siguiente derivada ademas dame la forma mas simplificada posible no escatimes en pasos, simplifica todo lo posible de la exprecion aunque tengas que hacer 100 pasos dame cada movimiento que hiciste y explica el por que de igual manera la respuesta correcta en este caso es: " + str(resultado) + ", asi que dime todos los pasos que llevaron a ese resultado: " + str(derivada)}
                    ],
                )
